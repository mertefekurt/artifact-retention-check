...
from artifact_retention_check import __version__
from artifact_retention_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from artifact_retention_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='artifact-retention-check',
        description=(
    ...
    parser.add_argument("input", nargs="?", type=Path, help='artifact retention policy')
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"artifact-retention-check: error: {exc}", file=sys.stderr)
        return 1
